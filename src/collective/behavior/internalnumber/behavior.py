...
def validateIndexValueUniqueness(context, portal_type, index_name, value):
    ...
    if context.portal_type != portal_type:
        ...
        if brains:
            raise Invalid(
                _(
                    u"This value '${value}' is already used in ${obj}",
                    mapping={
                        "obj": ", ".join(
                            [
...
def internal_number_default(data):
    ...
    ret = _evaluateExpression(
        data.context, settings.get("expr", ""), extra_expr_ctx={"number": settings.get("nb", 0)}, empty_expr_is_true=""
    )
    return ret


DefaultinternalNumber = widget.ComputedWidgetAttribute(
    internal_number_default, field=IInternalNumberBehavior["internal_number"]
)
